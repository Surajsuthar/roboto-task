...
// Query for fetching all blog posts for Algolia indexing
export const queryAllBlogPosts = defineQuery(`
...
export const queryCategoryPageData = defineQuery(`{
  ...
  "total": count(*[_type == "blog" && (seoHideFromLists != true) && references(*[_type == "category" && slug.current == $slug][0]._id)]),
  "posts": *[_type == "blog" && (seoHideFromLists != true) && references(*[_type == "category" && slug.current == $slug][0]._id)] | order(orderRank asc)[$offset...$end]{
    ${blogCardFragment}
  }
}`);
